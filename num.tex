...
\begin{remark*}[Reformulation of CG algorithm for $\tilde{A} \tilde{x} = \tilde{b}$]
  ...
  \TODO[Ask about the $\tilde{x_k}$ definition. Kazeev did write $\tilde{p}_{k-1}$ instead of $\tilde{p}_k$]
  \begin{mdframed}
    \begin{equation*}
    ...
    \end{equation*}
    For $k \in \N$, assuming $z_{k-1} = P^{-1} r_{k-1}$ and $v_k = C \tilde{p}_k$ have been evaluated, we define
    \begin{equation*}
      z_k = P^{-1} r_k \quad \text{ and } \quad v_{k+1} = C \tilde{p}_{k+1}
    \end{equation*}
  \end{mdframed}
\end{remark*}
\begin{algorithm}[The preconditioned CG method, PCG]~\\[\parskip]
  \label{alg:9.3}
  Given: $A, P \in \F^{n\times n}$ Hermitian positive definite, $b, x_0 \in \F^n$ such that $b - Ax_0 \neq 0$

  Initialize: set $r_0 = b - Ax_0,\ z_0 = P^{-1} r_0,\ v_1 = z_0$

  Iterate for $k = 1,2,\ldots$\::

  \hspace{2em} set $\tilde{\alpha}_k = \frac{r_{k-1}^* z_{k-1}}{v_k^* A v_k}$

  \hspace{2em} set $x_k = x_{k-1} + \tilde{\alpha}_k v_k$

  \hspace{2em} set $r_k = r_{k-1} - \tilde{\alpha}_k A v_k$

  \hspace{2em} if $r_k = 0$, then terminate

  \hspace{2em} set $z_k = P^{-1} r_k$

  \hspace{2em} set $\tilde{\beta}_k = \frac{r_k^* z_k}{r_{k-1}^* z_{k-1}}$

  \hspace{2em} set $v_{k+1} = z_k + \tilde{\beta}_k v_k$
\end{algorithm}
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Chebyshev method of Richardson type.}
    ...
    \item \textbf{Chebychev method of Krylov type}
      ...
      (We considered such a general form of a three-term recurrence relation for orthogonal polynomials in lecture 8, see right below Remark \ref{rem:8.1})

      Let us set $c_k = T_k(\tau)$ $\forall k \in \N_0$. As we showed in the proof of Lemma \ref{lem:8.2}A, we have $c_k = \frac{1}{2}(\mu^k + \mu^{-k})$ $\forall k \in \N$, where $\mu = \frac{\sqrt{\kappa}+1}{\sqrt{\kappa}-1}$. Let $\omega = \frac{1}{\Lambda + \lambda}$. Then
      \begin{equation*}
        \phi(t) = \tau - \frac{2t}{\Lambda - \lambda} = \tau (1 - \omega t) \quad \forall t \in \R
      \end{equation*}
      \TODO[Something doesn't add up here, i.e.]
      \textcolor{red}{
        \begin{gather*}
          \kappa = \frac{\Lambda}{\lambda},\ \tau = \frac{\kappa + 1}{\kappa - 1} = \frac{\Lambda + \lambda}{\Lambda - \lambda}\\
          \phi(t) = \tau - \frac{2t}{\Lambda - \lambda} = \tau (1 - \frac{2t}{\tau(\Lambda - \lambda)}) = \tau (1 - \frac{2t}{\Lambda + \lambda})\\
        \end{gather*}
      }
